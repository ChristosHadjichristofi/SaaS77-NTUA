    ...
    .then(() => {
        sequelize
            ...
            .then((result) => {
                ...
                let retrieveEventsPromise = models.Events.findAll({ raw: true, where: { id: 1 } })
                    .then(rows => {
                        /* if no rows returned then processed 0 events, create the row with id = 1 and counter = 0 */
                        if (rows.length == 0) return models.Events.create({ counter: 0 });
                        else {
                ...
                Promise.all([retrieveEventsPromise]).then(() => {
                    ...
                    const token = jwt.sign({ service: 'Graphs Service' }, process.env.SECRET_JWT, { expiresIn: '20s' });
